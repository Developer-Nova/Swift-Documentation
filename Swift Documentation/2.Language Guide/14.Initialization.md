...
다른 초기화와 마찬가지로 하위 클래스에서 상위 클래스 실패 가능한 초기화를 재정의할 수 있습니다.

또한 상위 클래스의 실패 불가능한 초기화를 하위 클래스의 실패 불가능한 초기화로 재정의할 수 있습니다.

이를 통해 상위 클래스의 초기화가 실패하더라도 초기화를 실패할 수 없는 하위 클래스를 정의할 수 있습니다.

#

실패 가능한 상위 클래스 초기화를 실패 불가능한 하위 클래스 초기화로 재정의하는 경우 상위 클래스 초기화를 위임하는 유일한 방법은 실패 가능한 상위 클래스 초기화의 결과를 강제로 언래핑하는 것입니다.

> Note    
> 실패 가능한 초기화를 실패 불가능한 초기화로 재정의 할 수 있지만 그 반대는 불가능합니다.

#

아래 예제는 `Document` 라는 클래스를 정의합니다.

이 클래스는 비어 있지 않은 문자열 값 또는 `nil` 은 가능하지만 빈 문자열은 불가능한, `name` 프로퍼티를 가지고 초기화될 수 있는 문서를 모델링 합니다.
...
다음 예제는 `AutomaticallyNamedDocument` 라는 `Document` 의 하위 클래스를 정의합니다.

`AutomaticallyNamedDocument` 하위 클래스는 `Document` 에 의해 도입된 지정 초기화를 모두 재정의합니다.

이러한 재정의는 인스턴스가 이름 없이 초기화되거나 빈 문자열이 `init(name:)` 초기화에 전달되는 경우 `AutomaticallyNamedDocument` 인스턴스가 `name` 의 초기값 `"[Untitled]"` 를 갖도록 보장합니다.
...
`AutomaticallyNamedDocument` 는 실패 불가능한 `init(name:)` 초기화로 상위 클래스의 실패 가능한 `init?(name:)` 초기화를 재정의 합니다.

`AutomaticallyNamedDocument` 는 상위 클래스와는 다른 방식으로 빈 문자열 케이스를 처리하기 때문에 초기화는 실패할 필요가 없으며 대신 실패할 수 없는 버전의 초기화를 제공합니다.

#

하위 클래스의 실패 불가능한 초기화 구현의 일부로 상위 클래스에서 실패 가능한 초기화를 호출하기 위해 초기화에서 강제 언래핑을 사용할 수 있습니다.

예를 들어 아래의 `UntitleDocument` 하위 클래스는 항상 `"[Untitled]"` 이름이 지정되며 상위 클래스의 실패 가능한 초기화 `init(name:)` 을 사용합니다.
...
이 경우 상위 클래스의 `init(name:)` 초기화가 이름으로 빈 문자열을 사용하여 호출되면 강제 언래핑 작업으로 인해 런타임 에러가 발생합니다.

하지만 문자열 상수로 호출되기 때문에 초기화가 실패하지 않으므로 이 경우 런타임 에러가 발생하지 않는 것을 알 수 있습니다.

<br>

### init! 실패 가능한 초기화 (The init! Failable Initializer)

일반적으로 `init` 키워드 뒤에 물음표를 배치하여 (`init?`) 적절한 타입의 옵셔널 인스턴스를 생성하는 실패 가능한 초기화를 정의합니다.

또는 적절한 타입의 암시적으로 언래핑된 옵셔널 인스턴스를 생성하는 실패 가능한 초기화를 정의할 수 있습니다.

물음표 대신에 `init` 키워드 뒤에 느낌표를 위치 시키면 됩니다. (`init!`)

#

`init?` 에서 `init!` 으로 또는 그반대로 위임할 수 있으며 `init?` 를 `init!` 으로 또는 그반대로 재정의 할 수 있습니다.

`init` 에서 `init!` 으로 위임할 수도 있지만 그렇게 하면 `init!` 초기화에 의해 초기화가 실패하는 경우 어설션이 트리거됩니다.

<br>

## 필수 초기화 (Required Initializers)

클래스의 모든 하위 클래스가 해당 초기화를 구현해야 함을 나타내기 위해 클래스 초기화 정의 앞에 `required` 수정자를 작성합니다.
...
또한 필수 초기화의 모든 하위 클래스 구현 전에 `required` 수정자를 작성하여 초기화 요구사항이 체인의 추가 하위 클래스에 적용됨을 나타내야 합니다.

필수 지정 초기화를 재정의할 때는 `override` 수정자를 작성하지 않습니다.
...
> Note    
> 상속된 초기화로 요구사항을 충족할 수 있는 경우 필수 초기화의 명시적인 구현을 제공할 필요가 없습니다.

<br>

## 클로저 또는 함수를 사용하여 기본 프로퍼티 값 설정 (Setting a Default Property Value with a Closure or Function)

저장 프로퍼티의 기본값에 사용자 정의 또는 설정이 필요한 경우 클로저나 전역 함수를 사용하여 해당 프로퍼티에 사용자 정의된 기본값을 제공할 수 있습니다.

프로퍼티가 속한 타입의 새로운 인스턴스가 초기화될 때마다 클로저 또는 함수가 호출되고 해당 반환값이 프로퍼티의 기본값으로 할당됩니다.

#

이러한 종류의 클로저 또는 함수는 일반적으로 프로퍼티와 동일한 타입의 임시값을 생성하고 해당 값을 원하는 초기 상태를 나타내도록 조정한 다음 프로퍼티의 기본값으로 사용할 임시값을 반환합니다.

#

다음은 클로저를 사용하여 기본 프로퍼티 값을 제공하는 방법에 대한 기본 개요 입니다.
...
클로저의 중괄호 끝에는 빈 소괄호 쌍이 옵니다.

이것은 Swift 가 즉시 클로저를 실행하도록 지시합니다.

이러한 소괄호를 생략하면 클로저의 반환값이 아닌 클로저 자체를 프로퍼티에 할당하려고 합니다.

> Note    
> 클로저를 사용하여 프로퍼티를 초기화하는 경우 클로저가 실행되는 시점에 인스턴스의 나머지 부분이 아직 초기화되지 않았다는 점을 기억해야 합니다.
> 이것은 해당 프로퍼티에 기본값이 있더라도 클로저 내부에서 다른 프로퍼티 값에 접근할 수 없음을 의미합니다.
> 또한 암시적 `self` 프로퍼티를 사용하거나 인스턴스 메서드를 호출할 수 없습니다.

#

아래 예제는 체스 게임용 보드를 모델링하는 `Chessboard` 라는 구조체를 정의합니다.

체스는 검은색과 흰색 사각형이 번갈아 나타나는 8 x 8 보드에서 진행됩니다.

<img width="450" src="https://github.com/Developer-Nova/Swift-Documentation/assets/123448121/5775d76c-7a73-40b7-a185-4562fccc8d7f">

이 게임 보드를 표현하기 위해 `Chessboard` 구조체는 64개의 `Bool` 값의 배열인 `boardColors` 라는 단일 프로퍼티가 있습니다.

배열의 `true` 값은 검은색 정사각형을 나타내고 `false` 값은 흰색 정사각형을 나타냅니다.

배열의 첫번째 항목은 게임 보드의 좌측상단 사각형을 나타내고 배열의 마지막 항목은 게임 보드의 우측하단 사각형을 나타냅니다.

#

`boardColors` 배열은 색상값을 설정하기 위해 클로저로 초기화됩니다.
...
새로운 `Chessboard` 인스턴스가 생성될 때마다 클로저가 실행되고 `boardColors` 의 기본값은 계산되어 반환됩니다.

위 예제의 클로저는 게임 보드의 각 사각형에 대한 적절한 색상을 계산하고 `temporaryBoard` 라는 임시 배열에 설정하며 설정이 완료되면 이 임시 배열을 클로저의 반환값으로 반환합니다.

반환된 배열값은 `boardColors` 에 저장되고 유틸리티 함수 `squareIsBlackAt(row:coloumn:)` 으로 조회할 수 있습니다.
